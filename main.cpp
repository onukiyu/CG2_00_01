	...
	ID3D12Device* device, D3D12_DESCRIPTOR_HEAP_TYPE heapType, UINT unmDescriptors, bool shaderVisible)
{
	...
	descriptorHeapDesc.NumDescriptors = unmDescriptors;
...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int)
{
	...
	HWND hwnd = CreateWindow(
		...
		L"LE2D_04_オオヌキ_ユウ", //タイトルバーの文字(なんでもいい)
	...
	swapChainDesc.Width = kClientWidth;     //画面の幅。ウインドウのクライアント領域を同じものにしておく
	swapChainDesc.Height = kClientHeight;   //画面の高さ。ウインドウのクライアント領域を同じものにしておく
	...
	//まず1つ目を作る。1つ目最初のところに作る。作る場所をこちらで指定してあげる必要がある
...
//Sprite用の頂点リソースを作る
	...
	*transformationMatrixData = MakeIdentity4x4();
	...
	for (uint32_t index = 0; index < kNumInstance; ++index) {
		instancingData[index].MVP = MakeIdentity4x4();
		...
		else
		{
			...
			*wvpData = worldViewProjectionMatrix;
			...
			for (uint32_t index = 0; index < kNumInstance; ++index) {
				...
				instancingData[index].MVP = worldViewProjectionMatrix;
			...
			/*bool DeptFunc(float currZ, float prevZ) {
				return currZ <= prevZ;
			}
